...
class Liblingua:
    """
    Official Python client for liblingua.
        ...
        self._session.headers.update(
            {
                ...
                "User-Agent":    "liblingua-python/1.0.0",
    ...
    def _get(
        ...
        try:
            ...
            raise LiblinguaError(f"Cannot connect to {self.base_url}: {exc}") from exc
        ...
        if not resp.ok:
            raise LiblinguaError(f"API error {resp.status_code}: {resp.text[:300]}")
        return resp
    ...
    def list_datasets(self) -> List[dict]:
        ...
        -------
        list[dict]
            Keys: language, record_count, audio_count, avg_quality,
            domain_dist, version, published_at.
        """
    ...
    def stream(
        ...
        *,
        ...
        validated_only : bool
            Only return human-validated translations (default True).
        min_quality : float, optional
            Minimum quality score in [0, 1].
        raw : bool
            Return unprocessed DB values — no split tag, no ISO codes,
            no text normalisation, no resolved audio URLs (default False).
        ...
        Examples
        --------
        >>> for rec in client.stream("grebo"):
    ...
    def load(
        ...
        *,
        ...
        language : str | list[str]
            One language code or a list of codes.
        validated_only : bool
        ...
        if isinstance(language, list):
            ...
            for lang in language:
                combined.extend(
                    self.stream(lang, validated_only=validated_only,
                                min_quality=min_quality, raw=raw)
                )
            return combined
        return list(
            self.stream(language, validated_only=validated_only,
                        min_quality=min_quality, raw=raw)
        )
    ...
    def download(
        ...
        *,
        ...
        if "filename=" in cd:
            ...
            filename = f"{language}{suffix}_liblingua_dataset.{ext}"
    ...
    def load_dataframe(self, language: Union[str, List[str]], **kwargs):
        """Load a dataset into a pandas DataFrame.

        Requires ``pandas``: ``pip install "liblingua[pandas]"``

        Examples
        --------
        >>> df = client.load_dataframe("kpelle")
        ...
        try:
            ...
            raise ImportError(
                'pandas is required.  Install with: pip install "liblingua[pandas]"'
            ) from exc
        return pd.DataFrame(self.load(language, **kwargs))

    def to_hf_dataset(self, language: Union[str, List[str]], **kwargs):
        """Load directly into a HuggingFace ``datasets.Dataset`` object.

        Requires ``datasets``: ``pip install "liblingua[huggingface]"``
        ...
        try:
            ...
            raise ImportError(
                'datasets is required.  Install with: pip install "liblingua[huggingface]"'
            ) from exc
    ...
    def __repr__(self) -> str:
        return f"Liblingua(base_url={self.base_url!r})"
